...
onEvent('block.registry', event => {
	...
	event.create('computation_matrix').model("cabin:block/computation_matrix").material("lantern").hardness(0.1).displayName('Computation Matrix').fullBlock(false).notSolid().box(1,1,1,15,15,15).waterlogged().opaque(false).lightLevel(7).renderType("translucent").item(i => { i.rarity(RARITY_UNCOMMON) })
	...
	reagent(0x05030A, 0x36234C, "obsidian", "Obsidian", F("#dusts/obsidian"), "create:powdered_obsidian")
